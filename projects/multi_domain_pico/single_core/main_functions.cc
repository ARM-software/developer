...
void handle_output(unsigned int i, int audio_score, int video_score) {
  int8_t average = compute_average(audio_score, video_score);
  printf("audio score = %f, video score = %f from i =  %f\n",
        static_cast<double>(audio_score), static_cast<double>(video_score),
...
void loop(unsigned int i) {
  ...
  handle_output(i, audio_score, video_score);
